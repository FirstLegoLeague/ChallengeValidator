 ...
 */
function verifyMission(mission, challenge) {
    var missionModel = model.createMissionModel(mission);
...
function checkString(object, field, model) {
    ...
    if (model.strings[key] === undefined) {
        output.error("Field '" + field + "' contains unknown string " + key);
        error.increase();
    } else {
        model.strings[key].used = true;
    }
}
...
function checkUnusedStrings(model) {
    for (var stringId in model.strings) {
        if (!model.strings[stringId].used) {
            output.error("String '" + stringId + "' not used anywhere");
            error.increase();
        }
    }
}
